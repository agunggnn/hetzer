...
test("buildReleaseMetadata prepares a new immutable version tag", () => {
    assert.deepEqual(buildReleaseMetadata({ version: "0.4.15", commit }), {
        version: "0.4.15",
        tag: "v0.4.15",
        tagExists: false,
    });
...
test("buildReleaseMetadata permits an idempotent rerun for the same commit", () => {
    assert.equal(buildReleaseMetadata({ version: "0.4.15", commit, taggedCommit: commit }).tagExists, true);
});
...
test("buildReleaseMetadata refuses malformed versions and tag movement", () => {
    ...
    assert.throws(() => buildReleaseMetadata({ version: "0.4.15", commit: "short" }), /full Git commit SHA/);
    assert.throws(
        () => buildReleaseMetadata({ version: "0.4.15", commit, taggedCommit: "b".repeat(40) }),
        /Refusing to move v0\.4\.15/,
    );
});
